...
void Network::bddChangeNet(Bdd f, vector<Node*>& intnode, vector<bool>& n_frg, vector<bool>& e_frg) //tomorow
{
    if(f == Bdd::null){ 
...
if(f1 == Bdd::one){
 ...
 //   cout << "f1 f0 is not constant" << endl;
        n_frg.push_back(true);
        bddChangeNet(f1,intnode,n_frg,e_frg);
        n_frg.pop_back();
    ...
    if(f0 == Bdd::one){
        ...
        n_frg.push_back(false);        
        bddChangeNet(f0,intnode,n_frg,e_frg);
        n_frg.pop_back();
        //cout << " return p3" << endl;   
        e_frg[varid] = false;     
        return;
...
void Network::calcOutFuncIm(Node* node)  //tomorow
{
    ...
    if(node->getType() == AND){
       // cout << "AND" <<endl;
        ...
        //outfuncs[node] = node->makeFuncIfNot();
        outfuncs[node] = tmp;
        return ;
    }
...
void Network::calcOutFuncIm_All(Node* node) //tomorow
{
    vector<Node*> fanin = node->getInput();
    for(auto i: fanin){
...
void Network::propagateCSPF(Node* node) {
    ...
    if(node->getType() == SOP) calcSOPcspf(node);
}
/* nodeのCSPFを設定 */
...
void Network::setCSPF_All_Im(Node* node, Bdd f0, Bdd f1) {   //tomorow
    clearCSPF();
    setCSPF_Im(node, f0, f1);
    for (auto no = ++primaryI.begin(); no != primaryI.end(); no++) setCSPF(*no);
}


void Network::calcSOPcspf(Node* node)  //tomorow
{
    //vector<Node*> nodelist = circ1->getnodelist();
    //assert(nodelist.size() != 0 );
    Bdd func_n = node->makeFuncIfNot();
    //cout << "node name is " << node->getName() <<endl; 
    Network* subnet = makeNetwork(func_n);

    //cout << "---------sub network----------- " << endl <<endl;
    vector<Node*> sublist = subnet->getPIlist();
    //unordered_map<void*, Bdd> outfuncs_c1 = circ1->getOutfunc();
    
      int count = 1;
    /*if(node->getInput().size()==2){
         count = 2;
      }else if(node->getInput().size()==1){
          count =3;
      }*/    
                                                  //inputの受け渡し
    for(auto i: node->getInput()){
      //cout << " x_" << count << " is " << node->getName() << " : " << i->getName() << endl;
      subnet->setOutFuncIm(sublist[count], outfuncs[i]);
      ++count;
    }
    
        subnet->calcOutFuncIm_All(subnet->getPOlist()[1]); //calcoutsunc subnetwork


    //cout << "---------sub network----------- " << endl <<endl;
    

    vector<Node*> sublist1 = subnet->getPIlist();

    
    //cout << "number of outfuncs is" << subnet->getOutfunc().size() << endl;
    //cout << "~~input~~" << endl;
    assert(sublist1.size() != 0);
    //cout << "num  : " << sublist1.size()-1 << endl;
    for (auto i: sublist1){
      if(i){
    ...
    /*vector<Node*> sublist2 = subnet->getnodelist();
    //cout << "~~intnode~~" << endl;
    ...
    vector<Node*> sublist3 = subnet->getPOlist();
    //cout << "~~output~~" << endl;
    ...
    if(outfuncs[node] == subnet->getOutfuncidx(subnet->getPOlist()[1])){
      //cout << node->getName() <<" is success!!" << endl;
    }else{
      //cout << node->getName() <<" is failed.. : type " << node ->getType() <<endl;
    }
  
    Bdd f0 = getCSPF_f0(node);
    Bdd f1 = getCSPF_f1(node);    
    subnet->setCSPF_All_Im(subnet->getPOlist()[1],f0,f1) ;
    
    //vector<Node*> test = ->getPIlist();
    //for(auto no: test){
      //if(no){
        //if(circ2->checkCSPF(no)){
          //cout << "true" << endl;
        //}else{
          //cout << "false" << endl;
        //}
      //}
    //}
    //cout << "cspf maybe success.." <<endl;

  vector<Node*> tmplist = subnet->getPIlist();
    ...
    for(auto i: node->getInput()){
      Bdd tmp_f0 = subnet->getCSPF_f0(tmplist[count_cspf]);
      Bdd tmp_f1 = subnet->getCSPF_f1(tmplist[count_cspf]);
      assert(tmp_f1 != Bdd::null && tmp_f0 != Bdd::null);
      setCon2CSPF(i, node, tmp_f0, tmp_f1);
      ++count_cspf;
    }
   return;
}
...
Network* makeNetwork(Bdd func)
{
    ...
    vector<bool> n_frg;
    vector<bool> e_frg(4, false);
    circ->bddChangeNet(func,intnode,n_frg, e_frg);   //OR以外のnodeを作る
