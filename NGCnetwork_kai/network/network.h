...
class Network
{
    ...
    void bddChangeNet(Bdd f, vector<Node*>& intnode, vector<bool>& n_frg, vector<bool>& e_frg);
	...
	((f & node2cspf[node].f0) == Bdd::zero);
     ...
     void setCSPF_Im(Node* node, Bdd f0, Bdd f1) { node2cspf[node].f0 = f0; node2cspf[node].f1 = f1; }
     Bdd getCSPF_f1(Node* node){return node2cspf[node].f1;}
     Bdd getCSPF_f0(Node* node){return node2cspf[node].f0;}   
     void setCSPF_All_Im(Node* node, Bdd f0, Bdd f1);
     void calcSOPcspf(Node* node);
     void setCon2CSPF(Node* fin, Node* node, Bdd f0, Bdd f1){con2cspf[make_pair(fin, node)].f0 = f0;con2cspf[make_pair(fin, node)].f1 = f1;}
     bool checkCSPF(Node* node){
         if(node2cspf[node].f1 != Bdd::null || node2cspf[node].f0 != Bdd::null){
             return true;
         }else{
             return false;
             }
